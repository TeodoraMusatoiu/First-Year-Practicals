...
class UndoableEditor extends Editor with UndoHistory {
    ...
    /** Command: Transpose the characters */
    ...
    override def cutCommand () = {
      ...
      var p = ed.point
      var m = ed.mark
      var string = ed.copStr
      
      lastChange = new Change {
         def undo() = {ed.insert(p, string)}
         def redo() = {ed.cut(oldp, oldm)}
      }
